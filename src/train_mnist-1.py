...
from src.train_engine import train_loss_ensemble, train_single
from model import mnist_model
import torch.optim as optim
import random
import torch.distributions as tdist
from src.util import init_log, set_seed, new_folder

parser = argparse.ArgumentParser(description='train ensemble')
parser.add_argument('--data', default='../data/mnist', type=str,
...
parser.add_argument('--save_model_path', default='../checkpoint/mnist', type=str,
                    help='path to dataset (e.g. data/')
...
parser.add_argument('--loss_type', default='norm_cos1', type=str,
                    help='ce, project_loss, project_loss_5_5, project_loss_2_5, project_loss_2_5_1, project_loss_2_5_new')
# parser.add_argument('--loss_type', default='project_loss_5/5', type=str,
#                     help='ce, project_loss, gal_loss, gal_dis_loss, gal_regular_loss, gal_dis_regular_loss, gal_regular_loss_new, gal_regular_loss_new1, gal_regular_loss_new2')
...
parser.add_argument('--para_flag', default=0, type=int,
                    help='')
parser.add_argument('--para_proj', default=1, type=int,
                    help='para before project_loss')
parser.add_argument('--para_norm', default=6, type=int,
                    help='para before norm_loss')
parser.add_argument('--para_cos', default=0.09, type=int,
...
def get_model_list(model_num):
    if args.path_suffix == 'resnet20':
        model_list = [mnist_model.resnet20() for i in range(model_num)]
        model_name = ['resnet20-'+str(i) for i in range(model_num)]
    if args.path_suffix == 'resnet26':
        model_list = [mnist_model.resnet26() for i in range(model_num)]
        model_name = ['resnet26-'+str(i) for i in range(model_num)]
    if args.path_suffix == 'resnet32':
        model_list = [mnist_model.resnet32() for i in range(model_num)]
        model_name = ['resnet32-'+str(i) for i in range(model_num)]
    if args.path_suffix == 'resnetmix':
        model_list = [mnist_model.resnet20(),
                      mnist_model.resnet20(),
                      mnist_model.resnet26(),
                      mnist_model.resnet26(),
                      mnist_model.resnet32()]
...
if __name__ == '__main__':
    ...
    data_train = datasets.MNIST(root="../data", transform=transform_train, train=True,
                                download=True
                                )
    data_train_noise = datasets.MNIST(root="../data", transform=transform_train_noise, train=True,
                                download=True
                                )
    data_test = datasets.MNIST(root="../data", transform=transform_test, train=False)
